...
namespace ReckoningCounter.BLL.Reckoning.Logic.GZQH
{
    /// <summary>
    /// 股指期货清算处理单元，错误编码2600-2699
    /// Create By:宋涛
    /// Create date:2009-08-26
    /// </summary>
    public class GZQHReckonUnit : ReckonUnitBase<QH_TodayEntrustTableInfo, FutureDealBackEntity, QH_TodayTradeTableInfo>
    {
        /// <summary>
        /// 是否已经完成当前委托清算（成交+撤单）
        /// </summary>
        ...
        #region Overrides of ReckonUnitBase<QhTodayEntrustTable,FutureDealBackEntity>
        ...
        protected override bool InternalInsertMessage(object message)
        {
            if (hasDoneDeal)
            {
                LogHelper.WriteDebug("GZQHOrderLogicFlow.InternalInsertMessage:委托已经完成，回报作废" + message);
                return false;
            }
            ...
            if (message is FutureDealBackEntity)
            {
                FutureDealBackEntity ido = (FutureDealBackEntity)message;
                id = ido.Id;
                orderId = ido.OrderNo;
                desc = ido.DescInfo();
            ...
            if (id == string.Empty)
            {
                LogHelper.WriteDebug("GZQHOrderLogicFlow.InternalInsertMessage: 空ID");
                return false;
            }

            if (HasAddId(id))
            {
                LogHelper.WriteDebug("GZQHOrderLogicFlow.InternalInsertMessage: 已添加ID" + id);
                return false;
            }
            ...
            {
                LogHelper.WriteDebug("GZQHOrderLogicFlow.InternalInsertMessage: 已处理ID" + id);
                return false;
            }
            ...
            LogHelper.WriteDebug("GZQHOrderLogicFlow.InternalInsertMessage" + desc + entrusNum);
        ...
        protected override void GetAccountID()
        {
            ...
            var buySellType = BuySellType == CommonObject.Types.TransactionDirection.Buying
                                  ? CommonObject.Types.TransactionDirection.Selling
                                  : CommonObject.Types.TransactionDirection.Buying;
                                              ...
                                              bool isErrorPriceCancelRpt, ref string strMessage)
        {
            ...
            LogHelper.WriteDebug(
                "------xxxxxx------开始进行股指期货撤单GZQHReckonUnit.InstantReckon_CancelOrder" +
            ...
            if (tet == null)
            {
                strMessage = "GT-2600:[股指期货撤单清算]无法获取委托对象，委托单号=" + EntrustNumber;
            ...
            if (ca == null)
            {
                string msg = "[股指期货撤单清算]资金冻结记录不存在." + entrustNumString;
            ...
            if (capMemory == null)
            {
                strMessage = "GT-2601:[股指期货撤单清算]资金帐户不存在:" + CapitalAccount;
            ...
            if (OpenCloseType != Types.FutureOpenCloseType.OpenPosition)
            {
                ...
                {
                    string format = "股指期货撤单清算-回报冻结量与数据库冻结量不相等[EntrustNumber={0},CancelOrderVolume={1},FreezeHoldAmount={2}]";
                ...
                try
                {
                    ...
                    if (holdMemory == null)
                    {
                        strMessage = "GT-2602:[股指期货撤单清算]持仓账户不存在:" + HoldingAccount;
                        LogHelper.WriteDebug(strMessage);
                        return false;
                    }
                ...
                {
                    strMessage = "GT-2603:[股指期货撤单清算]持仓处理异常:" + ex;
                    LogHelper.WriteDebug(strMessage);
            ...
            try
            {
                using (DbConnection connection = db.CreateConnection())
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        strMessage = "GT-2604:[股指期货撤单清算]数据库提交失败";
        ...
        private void ProcessDealCommit()
        {
            ...
            string format = "股指期货成交清算[EntrustNumber={0},DealBackCount={1}";
            ...
            {
                //如果清算没有成功，那么从保存的回报列表中删掉，下次CrashManger可以再次发送此回报过来
                LogHelper.WriteInfo("#####################GZQHReckonUnit.ProcessDealCommit成交清算失败:" + strMessage);
        ...
        public bool InstantReckon_Deal(List<FutureDealBackEntity> idoList, ref string strMessage,
                                       out QH_TodayEntrustTableInfo tet, out List<QH_TodayTradeTableInfo> tradeList)
        {
            ...
            string format = "即时清算GZQHOrderLogicFlow.InstantReckon_Deal[资金帐户={0},持仓帐户={1},柜台委托单号={2}]";
            ...
            if (hasDealedAmount == EntrustAmount)
            {
                msg = "即时清算GZQHOrderLogicFlow.InstantReckon_Deal-成交回报错误：当前委托已经清算完毕，成交回报无效！EntrustNumber=" +
                      EntrustNumber;
                LogHelper.WriteDebug(msg);
            ...
            List<FutureDealBackEntityEx> idoexList;
            GZQHDealSum dealSum;
            bool isSuccess = ComputeGZQHDealBacks(idoList, out idoexList, out dealSum, ref strMessage);
            if (!isSuccess)
            {
                strMessage = "ComputeGZQHDealBacks失败！";
                LogHelper.WriteDebug(strMessage);
                return false;
            }
            ...
            {
                msg = "即时清算GZQHReckonUnit.InstantReckon_Deal-成交回报错误：成交回报成交数量{0}+已清算的数量{1}>当前委托量{2}！EntrustNumber=" +
                      EntrustNumber;
            ...
            if (CurrencyType == -1)
                CurrencyType =
                    MCService.FuturesTradeRules.GetCurrencyTypeByCommodityCode(Code).CurrencyTypeID;
            ...
            if (tet == null)
            {
                strMessage = "GT-2605:[股指期货成交清算]无法获取委托对象，委托单号=" + EntrustNumber;
                LogHelper.WriteDebug(strMessage);
                return false;
            }
            ...
            if (OpenCloseType == Types.FutureOpenCloseType.OpenPosition)
            {
                return GZQHOpen_InstantReckon_Deal(idoexList, dealSum, ref strMessage, out tet, out tradeList);
            }
            else
            {
                return GZQHClose_InstantReckon_Deal(idoexList, dealSum, ref strMessage, out tet, out tradeList);
            }
        }
        ...
        private void LastCheckMargin(int capitalAccountId, int holdingAccountId)
        {
            ...
            string format = "GZQHReckonUnit.LastCheckMargin[CapitalAccountId={0},HoldingAccountId={1},Margin={2}]";
        ...
        #region 开仓清算
        private bool GZQHOpen_InstantReckon_Deal(List<FutureDealBackEntityEx> idoExList, GZQHDealSum dealSum,
                                                ...
                                                out List<QH_TodayTradeTableInfo> tradeList)
        {
            ...
            {
                strMessage = "GT-2699:[股指期货成交清算]无法获取资金冻结记录.";
            ...
            if (capMemory == null)
            {
                strMessage = "GT-2606:[股指期货成交清算]资金帐户不存在:" + CapitalAccount;

                return false;
            }

            QH_CapitalAccountTable_DeltaInfo capitalDelta = GZQHOpen_InstantReckon_PreCapitalProcess(dealSum, caf, dealMoneySum);
            capitalDelta.CapitalAccountLogoId = capMemory.Data.CapitalAccountLogoId;
            ...
            if (!isSuccess)
            {
                strMessage = "GT-2698:[股指期货成交清算]无法获取持仓记录.";
            ...
            {
                holdFreeze = GZQHOpen_InstantReckon_HoldingFreezeProcess(dealSum, ref isFirstHold);
            }

            tet = QHDataAccess.GetEntrustTable(EntrustNumber);
            var tet2 = tet;
            GZQHOpen_InstantReckon_EntrustProcess(dealSum, tet2);
            ...
            try
            {
                using (DbConnection connection = database.CreateConnection())
                {
                    ...
                    try
                    {
                        ...
                        if (HoldingTradingRule != 0)
                        {
                            QH_HoldAccountFreezeTableDal holdDal =
                                new QH_HoldAccountFreezeTableDal();
                            if (isFirstHold)
                    ...
                    catch (Exception ex)
                    {
                        transaction.Rollback();
                        LogHelper.WriteError(ex.Message, ex);
                        strMessage = "GT-2607:[股指期货成交清算]清算提交失败！委托单号" + EntrustNumber;
            ...
            isSuccess = GZQHOpen_InstantReckon_HoldingProcess(holdMemory, dealSum, HoldingTradingRule);
            ...
            {
                //一定要执行，记录下来等下次执行
                RescueManager.Instance.Record_GZQHOpen_InstantReckon_HoldingProcess(HoldingAccountId, dealSum, HoldingTradingRule);
            }
            ...
            {
                LogHelper.WriteInfo("GZQHOrderLogicFlow.InstantReckon_Deal2清算成交量错误");
            }
            else
            {
        ...
        private void GZQHOpen_InstantReckon_EntrustProcess(GZQHDealSum dealSum, QH_TodayEntrustTableInfo tet2)
        {
            //成交均价
        ...
        /// <returns></returns>
        private QH_CapitalAccountTable_DeltaInfo GZQHOpen_InstantReckon_PreCapitalProcess(GZQHDealSum dealSum, QH_CapitalAccountFreezeTableInfo caf, decimal dealMoneySum)
        {
        ...
        /// <returns></returns>
        private QHHoldMemoryTable GZQHOpen_InstantReckon_PreHoldingProcess(ref bool isSuccess,
                                                     ref string strMessage)
        {
            ...
            try
            {
                ...
                if (holdMemory == null)
                {
                    ...
                    if (hold != null)
                    {
                        string format = "GZQHOpen_InstantReckon_PreHoldingProcess数据库存在持仓，直接加载到内存表中[Code={0},BuySell={1},HoldAccount={2}]";
                    ...
                    else
                    {
                        ...
                        string format = "GZQHOpen_InstantReckon_PreHoldingProcess数据库不存在持仓，新建一个空的持仓，并加入到数据库和内存表中[Code={0},BuySell={1},HoldAccount={2}]";
                        string desc = string.Format(format, Code, BuySellType, HoldingAccount);
                        LogHelper.WriteDebug(desc);
                    ...
                    if (holdMemory == null)
                    {
                        LogHelper.WriteDebug("GZQHOpen_InstantReckon_PreHoldingProcess无法获取持仓记录");
                        isSuccess = false;
                        return null;
                    }
                }
            ...
            {
                strMessage = "GT-2608:[股指期货成交清算]持仓处理异常:" + ex;
                LogHelper.WriteError(ex.Message, ex);
            }
        ...
        /// <returns></returns>
        private QH_HoldAccountFreezeTableInfo GZQHOpen_InstantReckon_HoldingFreezeProcess(GZQHDealSum dealSum, ref bool isFirstHold)
        {
        ...
        private static bool GZQHOpen_InstantReckon_HoldingProcess(QHHoldMemoryTable holdMemory, GZQHDealSum dealSum, int holdingTradingRule)
        {
            bool isSuccess = holdMemory.ReadAndWrite(hold =>
                                                         {
                                                             ...
                                                             decimal costPrice = 0;
                                                             if (hold.BuySellTypeId == (int)CommonObject.Types.TransactionDirection.Buying)
                                                             {
                                                             ...
                                                             hold.TodayHoldAveragePrice = (hold.TodayHoldAveragePrice *
                                                                                           todayAmount +
                                                                                           dealSum.CapitalSumNoScale) /
                                                                                          (todayAmount +
                                                                                           dealSum.AmountSum);
        ...
        /// <param name="dealSum"></param>
        /// <returns></returns>
        public static bool DoGZQHOpen_HoldingRescue(int holdingAccoutnId, GZQHDealSum dealSum, int holdingTradingRule)
        {
            ...
            return GZQHOpen_InstantReckon_HoldingProcess(holdMemory, dealSum, holdingTradingRule);
        }
        ...
        private bool GZQHClose_InstantReckon_Deal(List<FutureDealBackEntityEx> idoExList, GZQHDealSum dealSum,
                                                       ref string strMessage, out QH_TodayEntrustTableInfo tet,
                                                       out List<QH_TodayTradeTableInfo> tradeList)
        {
            //平今/平历史
            ...
            if (capMemory == null)
            {
                strMessage = "GT-2609:[股指期货成交清算]资金帐户不存在:" + CapitalAccount;
                LogHelper.WriteDebug(strMessage);
                return false;
            }

            //资金表操作1
            QH_CapitalAccountTable_DeltaInfo capitalDelta = GZQHClose_InstantReckon_PreCapitalProcess(capitalFreeze, dealSum);
            capitalDelta.CapitalAccountLogoId = capMemory.Data.CapitalAccountLogoId;
            ...
            bool isSuccess = false;
            QHHoldMemoryTable holdMemory = null;
            holdMemory = GZQHClose_InstantReckon_PreHoldingProcess(ref isSuccess, ref strMessage);
            ...
            isSuccess = GetAllGZQH_ProfitLossAndMargin(idoExList, holdAccountTable, out profitLossSum, out marginSum, ref strMessage);
            if (!isSuccess)
            {
                strMessage = "GT-2697:[股指期货成交清算]平仓盈亏与保证金计算失败：" + strMessage;
            ...
            GZQHClose_InstantReckon_HoldingProcess(holdDelta, dealSum, marginSum);
            ...
            isSuccess = GetAllGZQH_FloatAndMarketProfitLoss(idoExList, holdAccountTable, out floatProfitLossSum,
                                                            out marketProfitLossSum, ref strMessage);

            if (!isSuccess)
            {
                strMessage = "GT-2696:[股指期货成交清算]浮动盈亏和盯市盈亏计算失败：" + strMessage;
            ...
            if (holdFreeze == null)
            {
                strMessage = "GT-2695:[股指期货成交清算]无法获取持仓冻结记录.";
                LogHelper.WriteInfo(strMessage);
            ...
            tet = QHDataAccess.GetEntrustTable(EntrustNumber);
            var tet2 = tet;
            GZQHClose_InstantReckon_EntrustProcess(dealSum, tet2, floatProfitLossSum, marketProfitLossSum);
            ...
            try
            {
                using (DbConnection connection = database.CreateConnection())
                {
                    ...
                    try
                    {
                        ...
                        //资金表操作3
                        GZQHClose_InstantReckon_CapitalAndFreezeProcess(tet2, profitLossSum, marginSum, tm, capitalDelta);
        ...
        private void GZQHClose_InstantReckon_HoldingProcess(QH_HoldAccountTableInfo_Delta holdDelta, GZQHDealSum dealSum, decimal marginSum)
        {
            //持仓表操作1.冻结量减去成交量
            //平今
        ...
        private void GZQHClose_InstantReckon_CapitalAndFreezeProcess(QH_TodayEntrustTableInfo tet2, decimal profitLossSum, decimal marginSum, ReckoningTransaction tm, QH_CapitalAccountTable_DeltaInfo capitalDelta)
        {
            //资金冻结操作3
        ...
        private void GZQHClose_InstantReckon_EntrustProcess(GZQHDealSum dealSum, QH_TodayEntrustTableInfo tet2, decimal floatProfitLossSum, decimal marketProfitLossSum)
        {
            //成交均价
            tet2.TradeAveragePrice =
                (tet2.TradeAveragePrice * tet2.TradeAmount +
        ...
        /// <param name="dealSum"></param>
        /// <returns></returns>
        private QH_CapitalAccountTable_DeltaInfo GZQHClose_InstantReckon_PreCapitalProcess(QH_CapitalAccountFreezeTableInfo capitalFreeze, GZQHDealSum dealSum)
        {
            //注意：为了简化处理，在清算期间不再进行冻结资金的补偿计算及多退少补的计算，统一在本次清算结束后
            //对资金冻结表进行检查校验，所以在清算内部，直接在资金冻结表上减去成交金额和成交费用
        ...
        /// <param name="strMessage"></param>
        /// <returns></returns>
        private QHHoldMemoryTable GZQHClose_InstantReckon_PreHoldingProcess(ref bool isSuccess,
                                                     ref string strMessage)
        {
            QHHoldMemoryTable holdMemory = null;
            isSuccess = false;
            var buySellType = BuySellType == CommonObject.Types.TransactionDirection.Buying
                                      ? (int)CommonObject.Types.TransactionDirection.Selling
                                      : (int)CommonObject.Types.TransactionDirection.Buying;
        ...
        #region  获取浮动和盯市盈亏
        private bool GetAllGZQH_FloatAndMarketProfitLoss(List<FutureDealBackEntityEx> idoexList, QH_HoldAccountTableInfo holdAccountTable, out decimal floatProfitLossSum, out decimal marketProfitLossSum, ref string strMessage)
        {
            bool isSuccess = false;
            ...
            foreach (var entityEx in idoexList)
            {
                ...
                if (OpenCloseType == Types.FutureOpenCloseType.CloseTodayPosition)
                {
                    isSuccess = GetGZQHCloseToday_FloatMarketProfitLoss(holdAccountTable, ido, ref strMessage, out floatProfitLoss,
                                                        out marketProfitLoss);
                ...
                if (OpenCloseType == Types.FutureOpenCloseType.ClosePosition)
                {
                    isSuccess = GetGZQHCloseHistory_FloatMarketProfitLoss(holdAccountTable, ido, ref strMessage, out floatProfitLoss,
                                                       out marketProfitLoss);
                }
        ...
        private bool GetGZQHCloseToday_FloatMarketProfitLoss(QH_HoldAccountTableInfo holdAccountTable, FutureDealBackEntity ido, ref string strMessage, out decimal floatProfitLoss, out decimal marketProfitLoss)
        {
            floatProfitLoss = 0;
            marketProfitLoss = 0;
        ...
        private bool GetGZQHCloseHistory_FloatMarketProfitLoss(QH_HoldAccountTableInfo holdAccountTable, FutureDealBackEntity ido, ref string strMessage, out decimal floatProfitLoss, out decimal marketProfitLoss)
        {
            floatProfitLoss = 0;
            marketProfitLoss = 0;
        ...
        private bool GetAllGZQH_ProfitLossAndMargin(List<FutureDealBackEntityEx> idoexList, QH_HoldAccountTableInfo holdAccountTable, out decimal profitLossSum, out decimal marginSum, ref string strMessage)
        {
            profitLossSum = 0;
            marginSum = 0;
            ...
            foreach (var entityEx in idoexList)
            {
                ...
                if (OpenCloseType == Types.FutureOpenCloseType.CloseTodayPosition)
                {
                    isSuccess = GetGZQHCloseToday_CapitalProfitAndLoss(holdAccountTable, ido, ref strMessage, out profitLoss,
                                                        out margin);
                ...
                if (OpenCloseType == Types.FutureOpenCloseType.ClosePosition)
                {
                    isSuccess = GetGZQHCloseHistory_CapitalProfitAndLoss(holdAccountTable, ido, ref strMessage, out profitLoss,
                                                       out margin);
                }
        ...
        /// <returns></returns>
        private bool GetGZQHCloseToday_CapitalProfitAndLoss(QH_HoldAccountTableInfo holdAccountTable, FutureDealBackEntity ido, ref string strMessage, out decimal profitLoss, out decimal margin)
        {
            profitLoss = 0;
            margin = 0;
            #region 获取平仓盈亏
            ...
            if (BuySellType == CommonObject.Types.TransactionDirection.Buying)
            {
        ...
        /// <param name="margin"></param>
        /// <returns></returns>
        private bool GetGZQHCloseHistory_CapitalProfitAndLoss(QH_HoldAccountTableInfo holdAccountTable, FutureDealBackEntity ido, ref string strMessage, out decimal profitLoss, out decimal margin)
        {
            profitLoss = 0;
            margin = 0;
        ...
        private void BuildTradeList(QH_TodayEntrustTableInfo tet2, ReckoningTransaction tm, List<FutureDealBackEntityEx> idoExList, List<QH_TodayTradeTableInfo> tradeList2)
        {
            foreach (var idoEx in idoExList)
            {
                ...
                var trade = QHCommonLogic.BuildGZQHDealRpt(tet2, ido, costResult, idoEx.DealCapital, idoEx.MarketProfitLoss, tm, dealRptType);
                if (trade != null)
                {
        ...
        /// <returns></returns>
        private List<string> GetIdList(List<FutureDealBackEntity> list)
        {
        ...
        private List<string> GetIDList2(List<FutureDealBackEntityEx> list)
        {
        ...
        /// 计算股指期货成交回报汇总信息
        /// </summary>
        /// <returns>股指期货成交回报汇总对象</returns>
        private bool ComputeGZQHDealBacks(List<FutureDealBackEntity> idos, out List<FutureDealBackEntityEx> idoexs,
                                          out GZQHDealSum dealSum, ref string strMessage)
        {
            idoexs = new List<FutureDealBackEntityEx>();
            dealSum = new GZQHDealSum();
            foreach (var ido in idos)
            {
                FutureDealBackEntityEx ex = new FutureDealBackEntityEx(ido);
                QHCostResult costResult = null;
                decimal dealCapital = 0;
                decimal dealCost = 0;
                bool isSuccess = ComputePreprocCapital(ido, Code, OpenCloseType, ref strMessage, ref dealCapital,
        ...
        private bool ComputePreprocCapital(FutureDealBackEntity ido, string code,
                                           ...
                                           ref decimal dealCost, out QHCostResult qhcr)
        {
            ...
            try
            {
                ...
                decimal unitMultiple = MCService.GetTradeUnitScale(code,
                                                                   (CommonObject.Types.UnitType)
                                                                   Enum.Parse(typeof(CommonObject.Types.UnitType),
                                                                              iut.ToString()));
                //保证金比例
                decimal futureBail = MCService.GetFutureBailScale(code) / 100;
                ...
                //预成交费用
                qhcr = MCService.ComputeGZQHCost(code, Convert.ToSingle(ido.DealPrice), dealamount,
                                                 (CommonObject.Types.UnitType)
                                                 Enum.Parse(typeof(CommonObject.Types.UnitType), iut.ToString()),
                                                 openCloseType);
                //预成交费用
            ...
            {
                strMessage = "GT-2610:[股指期货成交清算]开仓委托成交清算成交金额及费用计算异常.";
                LogHelper.WriteError(ex.Message, ex);
            }
            return result;
        }
    ...
    /// 股指期货成交回报扩展类
    /// </summary>
    public class FutureDealBackEntityEx : ReckonDealBackEx<FutureDealBackEntity, QHCostResult>
    {
        public FutureDealBackEntityEx(FutureDealBackEntity dealBack)
            : base(dealBack)
        {
        }
