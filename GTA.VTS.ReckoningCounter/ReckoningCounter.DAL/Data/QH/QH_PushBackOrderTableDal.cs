...
{
    /// <summary>
    /// 数据访问类 QH_PushBackOrderTableDal
    /// </summary>
    public class QH_PushBackOrderTableDal
    {
        /// <summary>
        /// 增加一条数据
        /// </summary>
        ...
        public void Add(string tradeNumber, string channelId, Database database)
        {
            ...
            strSql.Append("insert into QH_PushBackOrderTable(");
        ...
        public void Delete(string tradeNumber, Database database)
        {
            Database db = database;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete QH_PushBackOrderTable ");
        ...
        /// 根据交易编号更新通道号
        /// </summary>
        /// <param name="tradeNumber">要更新的交易编号单个时以'33223'传递</param>
        ...
        public void Update(string tradeNumber, string channelID, Database database, DbTransaction transaction)
        {
            Database db = database;
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat("UPDATE [dbo].[QH_PushBackOrderTable]   SET [ChannelID] ='{0}'", channelID);
        ...
        public int DeleteNotPushToday(string channelID)
        {
            ...
            Database db = DatabaseFactory.CreateDatabase();
            StringBuilder strSql = new StringBuilder();
            strSql.Append("  delete QH_PushBackOrderTable where TradeNumber not in( ");
            strSql.AppendFormat(" select TradeNumber from dbo.QH_HistoryTradeTable where tradetime>='{0}' and TradeTime<'{1}'", startTime, endTime);
            strSql.Append("  union all");
            strSql.AppendFormat("  select TradeNumber from dbo.QH_TodayTradeTable where tradetime>='{0}' and TradeTime<'{1}'", startTime, endTime);
            strSql.AppendFormat("   ) and ChannelID='{0}' ", channelID.Trim());
            strSql.AppendFormat(" select count(1) from  dbo.QH_PushBackOrderTable where  ChannelID='{0}'", channelID.Trim());
            DbCommand dbCommand = db.GetSqlStringCommand(strSql.ToString());
            int cmdresult = 0;
        ...
        private string GetDistincEntrustNumberSqlScript()
        {
            ...
            strSql.Append("select EntrustNumber from QH_TodayTradeTable where tradenumber in(select tradenumber from QH_PushBackOrderTable where channelid='{0}' )");
            strSql.Append(" union all ");
            strSql.Append(" select EntrustNumber from QH_HistoryTradeTable where tradenumber in(select tradenumber from QH_PushBackOrderTable where channelid='{0}' )");
            strSql.Append(" )  as t ");
        ...
        private string GetEntrustListByEnturstNoSqlScript()
        {
            ...
            sqlStr.Append(" select   [EntrustNumber],[PortfolioLogo],[EntrustPrice],[EntrustAmount],[TradeAmount]");
            sqlStr.Append(" ,[TradeAveragePrice],[CancelAmount],[CancelLogo],[BuySellTypeId],[CurrencyTypeId]");
            sqlStr.Append(" ,[OpenCloseTypeId],[TradeUnitId],[OrderStatusId],[ContractCode],[TradeAccount]");
            sqlStr.Append(" ,[CapitalAccount],[IsMarketValue],[OrderMessage],[CallbackChannelId],[McOrderId]");
            sqlStr.Append(" ,[CloseFloatProfitLoss],[CloseMarketProfitLoss],[OfferTime],[EntrustTime]");
            sqlStr.Append("   from dbo.[QH_TodayEntrustTable]   where EntrustNumber in({0}) ");
            sqlStr.Append("  union all ");
            sqlStr.Append(" select  [EntrustNumber],[PortfolioLogo],[EntrustPrice],[EntrustAmount],[TradeAmount]");
            sqlStr.Append(" ,[TradeAveragePrice],[CancelAmount],[CancelLogo]=cast(0 as bit),[BuySellTypeId],[CurrencyTypeId]");
            sqlStr.Append(" ,[OpenCloseTypeId],[TradeUnitId],[OrderStatusId],[ContractCode],[TradeAccount]");
            sqlStr.Append(" ,[CapitalAccount],[IsMarketValue],[OrderMessage],[CallbackChannelId]='',[McOrderId]");
            sqlStr.Append(" ,[CloseFloatProfitLoss],[CloseMarketProfitLoss],[OfferTime],[EntrustTime]");
            sqlStr.Append("   from dbo.QH_HistoryEntrustTable   where EntrustNumber in({0}) ");
        ...
        private string GetTodayAndHistroyTradeNumberString(string entrustNumberList)
        {
            ...
            sb.Append("  from QH_TodayTradeTable ");
            sb.AppendFormat(" where  EntrustNumber in( {0} )", entrustNumberList);
            sb.Append(" union  all");
            sb.Append(" select TradeNumber ");
            sb.Append(" from QH_HistoryTradeTable ");
            sb.AppendFormat(" where  EntrustNumber in( {0} )", entrustNumberList);
            Database db = DatabaseFactory.CreateDatabase();
