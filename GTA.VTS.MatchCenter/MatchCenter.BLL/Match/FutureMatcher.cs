...
namespace MatchCenter.BLL
{
    ...
    public class FutureMatcher
    {
        ...
        /// 撤单存储区
        /// </summary>
        private QueueBufferBase<FutureDataOrderEntity> CancelBufferEntity;
        /// <summary>
        /// 缓冲区
        /// </summary>
        public QueueBufferBase<FutureDataOrderEntity> EntityBuffer;

        /// <summary>
        ///  委托单不在交易价格范围，发生熔断 （限废单）内部撤单事件
        /// </summary>
        private QueueBufferBase<FutureFuseEntity> RejectBuffer;
        ...
        ///// 存储【买限价】委托单
        ///// </summary>
        //private MatchStockFutureData buyOrder;
        ///// <summary>
        ///// 存储【买市价】委托单
        ///// </summary>
        //private MatchStockFutureData buyMarketPriceOrder;
        ///// <summary>
        ///// 存储【卖限价】委托单
        ///// </summary>
        //private MatchStockFutureData sellOrder;
        ///// <summary>
        ///// 存储【卖市价】委托单
        ///// </summary>
        //private MatchStockFutureData sellMarketPriceOrder;

        /// <summary>
        /// 存储【买限价】委托单
        /// </summary>
        private EntrustOrderData<FutureDataOrderEntity> buyOrder;
        /// <summary>
        /// 存储【买市价】委托单
        /// </summary>
        private EntrustOrderData<FutureDataOrderEntity> buyMarketPriceOrder;
        /// <summary>
        /// 存储【卖限价】委托单
        /// </summary>
        private EntrustOrderData<FutureDataOrderEntity> sellOrder;
        /// <summary>
        /// 存储【卖市价】委托单
        /// </summary>
        private EntrustOrderData<FutureDataOrderEntity> sellMarketPriceOrder;
        ...
        public FutureMatcher(string code)
        {
            ...
            //buyMarketPriceOrder = new MatchStockFutureData(Types.TransactionDirection.Buying);
            //buyOrder = new MatchStockFutureData(Types.TransactionDirection.Buying);

            //sellMarketPriceOrder = new MatchStockFutureData(Types.TransactionDirection.Selling);
            //sellOrder = new MatchStockFutureData(Types.TransactionDirection.Selling);
            buyMarketPriceOrder = new EntrustOrderData<FutureDataOrderEntity>(Types.TransactionDirection.Buying);
            buyOrder = new EntrustOrderData<FutureDataOrderEntity>(Types.TransactionDirection.Buying);

            sellMarketPriceOrder = new EntrustOrderData<FutureDataOrderEntity>(Types.TransactionDirection.Selling);
            sellOrder = new EntrustOrderData<FutureDataOrderEntity>(Types.TransactionDirection.Selling);

            EntityBuffer = new QueueBufferBase<FutureDataOrderEntity>();
            ...
            {
                MatchCodeDictionary.funtureMatchCodeDic[code] = null;
                MatchCodeDictionary.funtureMatchCodeDic.Remove(code);
            }
            MatchCodeDictionary.funtureMatchCodeDic.Add(matchCode, FutRealtimeMarketChangeEvent);


            CancelBufferEntity = new QueueBufferBase<FutureDataOrderEntity>();
            CancelBufferEntity.QueueItemProcessEvent += ProcessCanceBussiness;
            RejectBuffer = new QueueBufferBase<FutureFuseEntity>();
            RejectBuffer.QueueItemProcessEvent += ProcessRejustBussiness;
        ...
        public void AcceptOrder(FutureDataOrderEntity model)
        {
            try
            {
                //撮合中心委托单不能为空
                ...
                {
                    ////撮合中心行情数据
                    FutData data = RealtimeMarketService.GetRealTimeFutureDataByContractCode(matchCode);
                    if (data != null)
                    {
                        model.OrderPrice = (decimal)data.Lasttrade;
                    }
                    else
                    {
                    ...
                    if (EntityBuffer != null)
                    {
                        ...
                        FutureDataOrderDataAccess.Add(model);
                ...
                if (isPrice == false)
                {
                    ...
                    RejectBuffer.InsertQueueItem(fuseEntity);
            ...
            {
                LogHelper.WriteError("接收股指期货下单委托单操作异常" + ex.Message, ex);
            }
        }
        ...
        public int CancelOrder(CancelEntity model)
        {
            ...
            string sholdercode = "";
            var entry = FutureDataOrderDataAccess.GetFutureEntityByOrderNo(model.OldOrderNo.Trim());
            if (entry != null)
            {
            ...
            try
            {
                ...
                FutureDataOrderDataAccess.Update(model.OldOrderNo);//更新数据库
                //上下文不能为空
                if (context == null)
                {
                    LogHelper.WriteError("CH-500:期货撤单时通道上下文为空,撤单回推缓存到回推队列中", new Exception(model.OldOrderNo));
                    //保存到缓存回推队列中 还没有实现
                    TradePushBackImpl.Instanse.SaveSICancelBack(dataEntity);
                    return 1;
                }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ProcessBussiness(object sender, QueueItemHandleEventArg<FutureDataOrderEntity> e)
        {
            if (e.Item == null)
            {
                return;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void FutRealtimeMarketChangeEvent(object sender, FutDataChangeEventArg e)
        {
        ...
        private void ProcessMatch(FutData hqData)
        {
        ...
        /// <returns></returns>
        private FutureMarketEntity GetMarketEntity(FutData hqExData)
        {
            if (hqExData == null)
            {
                return null;
            ...
            MarketEntity.UpPrice = (decimal)hqExData.UpperLimitPrice;
            MarketEntity.LowerPrice = (decimal)hqExData.LowerLimitPrice;
            ...
            MarketEntity.HQReachTime = Convert.ToDateTime(hqExData.Time);//DateTime.Now;

            //if (hqExData.CodeKey == "IF0909" || hqExData.CodeKey == "IF0912" || hqExData.CodeKey == "IF1003")
            //{
            //    string msg = "内部组装时==" + hqExData.CodeKey + "行情到达：{0}  行情时间：{1}\r\n成交价：{2}\r\n";
            //    msg += "卖一：{3}--{4}   卖五：{5}--{6}\r\n  ";
            //    msg += "买一：{7}--{8}   买五：{9}--{10}\r\n  ";
            //    string wrmsg = string.Format(msg, DateTime.Now, MarketEntity.HQReachTime, MarketEntity.LastPrice, MarketEntity.SellFirstPrice, MarketEntity.SellFirstVolume, MarketEntity.SellFivePrice, MarketEntity.SellFiveVolume
            //                              , MarketEntity.BuyFirstPrice, MarketEntity.BuyFirstVolume, MarketEntity.BuyFivePrice, MarketEntity.BuyFiveVolume);
            //    LogHelper.WriteDebug(wrmsg);
            //}
            return MarketEntity;
        }
        ...
        private void MatchMarkPriceSell(FutureMarketEntity hqData)
        {
            ...
            //撮合中心撮合实体
            Dictionary<string, List<FutureDataOrderEntity>> orderDic = sellMarketPriceOrder.GetAcceptEntitys();
            //撮合中心撮合实体不能为空
            if (orderDic != null)
            {
                foreach (var item in orderDic)
                {
                    if (item.Value != null)
                    {
                        foreach (FutureDataOrderEntity entity in item.Value)
                        {
            ...
            //List<FutureDataOrderEntity> matchEntitys = sellMarketPriceOrder.GetAcceptEntitys();
            //if (!Utils.IsNullOrEmpty(matchEntitys))
            //{
            //    foreach (FutureDataOrderEntity entity in matchEntitys)
            //    {
        ...
        /// <param name="hqData">行情实体</param>
        private void MatchMarkPriceSell(FutureDataOrderEntity orderEntity, FutureMarketEntity hqData)
        {
            //实体不能为空
            ...
            //LogHelper.WriteDebug("---->【股指期货】开始撮合卖市价委托");
        ...
        private void MatchSell(FutureMarketEntity hqData)
        {
            ...
            #region update 2010-01-08
            //撮合中心撮合实体
            Dictionary<string, List<FutureDataOrderEntity>> orderDic = sellOrder.GetAcceptEntitys();
            //撮合中心撮合实体不能为空
            ...
            //List<FutureDataOrderEntity> matchEntitys = sellOrder.GetAcceptEntitys();
            //if (Utils.IsNullOrEmpty(matchEntitys))
            //{
            //    return;
            //}

            //Dictionary<string, List<FutureDataOrderEntity>> orderDic = new Dictionary<string, List<FutureDataOrderEntity>>();
            ...
            //    else
            //    {
            //        List<FutureDataOrderEntity> list = new List<FutureDataOrderEntity>();
            //        list.Add(model);
            ...
            {
                List<FutureDataOrderEntity> orderList = sholderCodeModel.Value;
                ...
                foreach (FutureDataOrderEntity item in orderList)
                {
        ...
        private void MatchSell(FutureDataOrderEntity orderEntity, FutureMarketEntity hqData, ref string marketVolueOrderNo)
        {
            if (orderEntity == null || hqData == null)
            {
                return;
            ...
            //LogHelper.WriteDebug("---->【股指期货】开始撮合卖限价委托");
            //LogHelper.WriteDebug("----> 行情: LastVolume=" + hqData.LastVolume + ",LastPrice=" + hqData.LastPrice + ";委托: OrderVolume=" + orderEntity.OrderVolume + ",OrderPrice=" + orderEntity.OrderPrice);
            ...
            if (hqData.LastVolume > 0.00m)
            {
                ...
                if (orderEntity.OrderPrice == hqData.LastPrice)
                {
                    ...
                    FutureDataOrderDataAccess.Update(orderEntity.OrderNo, orderEntity.MarketVolumeNo, orderEntity.MarkLeft);
                    FutureDataOrderDataAccess.Update(orderEntity.MarketVolumeNo, orderEntity.MarketVolumeNo, orderEntity.MarkLeft);//更新关联的委托市场存量
            ...
            if (orderEntity.OrderVolume > 0.00m)
            {
                ...
                {                    //用户编号+价格+委托编号
                    ...
                    FutureDataOrderDataAccess.Update(orderEntity.OrderNo, marketVolueOrderNo, marketVolume);
                }
        ...
        private void MatchMarkPriceBuy(FutureMarketEntity hqData)
        {
            ...
            #region update 2010-01-08
            //撮合中心撮合实体
            Dictionary<string, List<FutureDataOrderEntity>> orderDic = buyMarketPriceOrder.GetAcceptEntitys();
            //撮合中心撮合实体不能为空
            if (orderDic != null)
            {
                foreach (var item in orderDic)
            ...
            //List<FutureDataOrderEntity> matchEntitys = buyMarketPriceOrder.GetAcceptEntitys();
        ...
        /// <param name="hqData">行情数据</param>
        private void MatchMarkPriceBuy(FutureDataOrderEntity orderEntity, FutureMarketEntity hqData)
        {
            //实体不能为空
            if (orderEntity == null || hqData == null)
            {
            ...
            //LogHelper.WriteDebug("---->【股指期货】开始撮合买市价委托");
            //LogHelper.WriteDebug("----> 行情: LastVolume=" + hqData.LastVolume + ",LastPrice=" + hqData.LastPrice + ";委托: OrderVolume=" + orderEntity.OrderVolume + ",OrderPrice=" + orderEntity.OrderPrice);
        ...
        private void MatchBuy(FutureMarketEntity hqData)
        {
            ...
            #region update 2010-01-08
            //撮合中心撮合实体
            Dictionary<string, List<FutureDataOrderEntity>> orderDic = buyOrder.GetAcceptEntitys();
            //撮合中心撮合实体不能为空
            if (orderDic == null || orderDic.Count <= 0)
            {
                return;
            ...
            //List<FutureDataOrderEntity> matchEntitys = buyOrder.GetAcceptEntitys();
            //if (matchEntitys == null)
            //{
            //    return;
            //}
        ...
        private void MatchBuy(FutureDataOrderEntity orderEntity, FutureMarketEntity hqData, ref string marketVolueOrderNo)
        {
            if (orderEntity == null || hqData == null)
            {
                return;
            ...
            //LogHelper.WriteDebug("---->【股指期货】开始撮合买限价委托");
            //LogHelper.WriteDebug("----> 行情: LastVolume=" + hqData.LastVolume + ",LastPrice=" + hqData.LastPrice + ";委托: OrderVolume=" + orderEntity.OrderVolume + ",OrderPrice=" + orderEntity.OrderPrice);
            ...
            if (hqData.LastVolume > 0.00m)
            {
                ...
                if (orderEntity.OrderPrice == hqData.LastPrice)
                {
                    ...
                    FutureDataOrderDataAccess.Update(orderEntity.OrderNo, marketVolueOrderNo, orderEntity.MarkLeft);
                    FutureDataOrderDataAccess.Update(orderEntity.MarketVolumeNo, orderEntity.MarketVolumeNo, orderEntity.MarkLeft);//更新关联的委托市场存量
                    if (orderEntity.OrderVolume <= 0)
                    {
                        return;
        ...
        private void InsertDealEntity(FutureDataOrderEntity entity, decimal price, decimal volume)
        {
            ...
            var deal = new FutureDealBackEntity();
            //id
            deal.Id = code;
            //撮合中心通道号码
            ...
            if (FutureDealOrderDataAccess.Add(deal) == 1)
            {
                ...
                string showMesg = string.Format(GenerateInfo.CH_D002, GenerateInfo.CH_D02, deal.OrderNo, matchCode, deal.DealTime,
                ...
                FutureDataOrderDataAccess.Update(entity.OrderNo, volume);
                MatchCenterManager.Instance.matchCenterBackService.futureDealOrderCacheList.InsertQueueItem(deal);
        ...
        #region 7.委托单不在交易价格范围，发生熔断 （限废单）内部撤单事件
        /// <summary>
        /// 7.委托单不在交易价格范围，发生熔断 （限废单）内部撤单事件
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ProcessRejustBussiness(object sender, QueueItemHandleEventArg<FutureFuseEntity> e)
        {
            FutureFuseEntity model = e.Item;
            if (model == null)
            {
                return;
            }
            //RejustAcceptOrder(e.Item);     
            if (model.Model == null || string.IsNullOrEmpty(model.Model.ChannelNo))
            {
                return;
            }
            var dataEntity = new ResultDataEntity();
            dataEntity.OrderNo = model.Model.OrderNo;

            FutData data = RealtimeMarketService.GetStaticRealtimeServiceNotEvent.GetFutData(matchCode);
            if (data == null)
                return;
            decimal price = (decimal)data.Lasttrade;
            //发生熔断，委托单价格不在交易价格范围之内
            if (model.FuseMark == 1)
            {
                dataEntity.Message = "CH-0020:发生熔断，委托单价格不在交易价格范围之内,价格为" + price;
            }
            //委托单价格不在交易价格范围之内
            else
            {
                if (data.PreSettlementPrice < 0.00)
                {
                    dataEntity.Message = "CH-0022:" + matchCode + "行情昨日结算价不存在！";
                }
                else
                {
                    dataEntity.Message = "CH-0021:委托单价格不在交易价格范围之内,价格为" + price;
                }
            }
            OperationContext context = null;
            if (model.Model.ChannelNo != null)
            {
                if (MatchCenterManager.Instance.OperationContexts.ContainsKey(model.Model.ChannelNo))
                {
                    context = MatchCenterManager.Instance.OperationContexts[model.Model.ChannelNo];
                }
            }
            //撮合中心上下文不能为空
            if (context == null)
            ...
            try
            {
                ...
                if (callback != null)
                {
                    var md = new MatchResultDelegate(callback.ProcessStockIndexFuturesOrderRpt);
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ProcessCanceBussiness(object sender, QueueItemHandleEventArg<FutureDataOrderEntity> e)
        {
            FutureDataOrderEntity model = e.Item;
            if (model == null)
            {
                return;
            }
